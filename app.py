...
if tab == "Growth Trajectory":
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title='Revenue ($M)',
            title_font=dict(color=light_green),  # FIXED: title_font instead of titlefont
            tickfont=dict(color=light_green),
            gridcolor='#F3F4F6'
        ),
        yaxis2=dict(
            title='Company Valuation ($M)',
            title_font=dict(color=dark_green),  # FIXED
    ...
    for _, dc in active_dcs.iterrows():
        folium.CircleMarker(
            ...
            fillOpacity=0.7,
            weight=3,
            popup=f"{dc['name']}<br>{dc['location']}<br>Year: {dc['year']}<br>Size: {dc['sqft']:,} sq ft<br>GPUs: {dc['gpus']:,}<br>Status: {dc['status']}"
        ).add_to(m)
...
# ===================
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title='GPU Count',
            title_font=dict(color='#10B981'),  # FIXED: title_font instead of titlefont
            tickfont=dict(color='#10B981')
        ),
        yaxis2=dict(
            title='Customers',
            title_font=dict(color='#047857'),  # FIXED
